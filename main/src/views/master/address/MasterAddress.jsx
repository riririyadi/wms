...
export default class MasterAddress extends React.Component {
    ...
    _requery() {
        ...
        for (var i = 0; i < 65; i++) {
            listData.push({
                code: "ADDR000-00" + i,
                name: "Address " + i,
                street: "Jalan perjuangan No. " + i,
                number: (i + 17),
                village: "Village " + i,
                latlong: "89.0000,12.0000" + i
            });
        }
        this.setState({
            gridData: {
    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 150, -1, -1, 70, 120, 120],
                    columns: ["No", "Address Code", "Address Name", "Street", "Number", "Village", "Lat Long"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.code, item.name, item.street, item.number, item.village, item.latlong];
                    }
                }
                onRowSelected={(index) => {
