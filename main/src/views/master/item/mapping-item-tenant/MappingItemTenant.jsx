...
export default class MappingItemTenant extends React.Component {
    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 100, 100 , 100, 100, 100, 100, 100],
                    columns: ["No", "Item Code", "Item Description", "Tenant Name", "Created By", "Created Date", "Modified By", "Modified Date"
                    ]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.code, item.desc, item.tenant, item.createdBy, item.createdDate, item.modifiedBy, item.modifiedDate];
                    }
                }
                onRowSelected={(index) => {
