...
export default class MappingUserTenant extends React.Component {
        ...
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    ...
                    columns: ["No", "User Name", "Tenant Name", "Created By", "Created Date", "Modified By", "Modified Date"
                    ]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.username, item.tenant, item.createdBy, item.createdDate, item.modifiedBy, item.modifiedDate];
                    }
                }
                onRowSelected={(index) => {
