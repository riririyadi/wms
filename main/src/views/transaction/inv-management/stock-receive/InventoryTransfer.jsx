...
export default class StockReceive extends React.Component {
        ...
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    ...
                    columns: ["No", "Trx Number", "Created Date", "Type", "Status", "Synch Status", "Business Unit", "Warehouse"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.number, item.createdDate, item.type, item.status, item.synchStatus, item.buName, item.whName];
                    }
                }
                onRowSelected={(index) => {
