...
export default class SettingReorderPoint extends React.Component {
    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 150, 150, -1],
                    columns: ["No", "Process", "Activity", "Value"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.process, item.activity, item.selectValue];
                    }
                }
                onRowSelected={(index) => {
