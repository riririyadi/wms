...
export default class MasterDistrict extends React.Component {
    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 150, -1, -1],
                    columns: ["No", "District Code", "District Name", "Province"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.code, item.name, item.province];
                    }
                }
                onRowSelected={(index) => {
