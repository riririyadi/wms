...
export default class MasterSLATime extends React.Component {
        ...
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    ...
                    columns: ["No", "Process Code", "Process Name", "SLA Time (Minutes)"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.processCode, item.processName, item.slaTime];
                    }
                }
                onRowSelected={(index) => {
