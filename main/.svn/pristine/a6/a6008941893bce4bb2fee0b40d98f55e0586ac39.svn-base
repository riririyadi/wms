...
export default class MasterUser extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            gridData: {
    ...
    _requery() {
        ...
        for (var i = 0; i < 8; i++) {
            listData.push({
                userName: "User " + i,
                employeeNumber: "20220412" + i,
                divisionName: "Division Name " + i,
                phoneNumber: "08532003001" + i,
        ...
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    ...
                    columns: ["No", "User Name", "Employee Number", "Division Name", "Phone Number", "Email", "Inactive", "Password"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.userName, item.employeeNumber, item.divisionName, item.phoneNumber, item.email, item.inactive, item.password];
                    }
                }
                onRowSelected={(index) => {
