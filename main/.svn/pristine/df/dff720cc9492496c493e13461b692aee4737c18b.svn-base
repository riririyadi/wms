...
export default class MasterSalesChannel extends React.Component {
    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 120, -1, 150, 120, 120],
                    columns: ["No", "Channel Code", "Channel Name", "Type", "Warehouse", "Storage Code"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.code, item.name, item.type, item.warehouse, item.storage];
                    }
                }
                onRowSelected={(index) => {
