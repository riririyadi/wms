...
export default class BufferStock extends React.Component {
    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 150, 100, -1],
                    columns: ["No", "Warehouse Code", "Item Code", "Qty"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.warehouseCode, item.itemCode, item.qty];
                    }
                }
                onRowSelected={(index) => {
