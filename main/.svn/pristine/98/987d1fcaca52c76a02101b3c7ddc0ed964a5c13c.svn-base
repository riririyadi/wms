...
export default class MasterVillage extends React.Component {
    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 180, -1, -1, 80],
                    columns: ["No", "Village Code", "Village Name", "Sub District Name", "Zip Code"]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.code, item.name, item.subdistrict, item.zipcode];
                    }
                }
                onRowSelected={(index) => {
