    ...
    _buildGrid() {
        return (
            <AzecGrid ref={this.refGrid}
                header={{
                    columnWidth: [40, 100, -1, 90, 130, 150, 80, 100],
                    columns: ["No", "Item Code", "Item Description", "Item Unit", "Item Type", "Item Group", "Tax Code", "Brand"
                    ]
                }}
                renderRow={
                    (item, index) => {
                        return [(index + 1), item.code, item.desc, item.unit, item.type, item.group, item.taxCode, item.brand];
                    }
                }
                onRowSelected={(index) => {
